...
def tensor2im(image_tensor, imtype=np.uint8):
	...
	image_numpy = Image.fromarray(np.uint8(image_numpy))
	return image_numpy

def tensor2im_restored(image_tensor, imtype=np.uint8):
    image_numpy = image_tensor[0].cpu().float().numpy()
    # image_numpy = (np.transpose(image_numpy, (1, 2, 0)) + 1) / 2.0 * 255.0
    image_numpy = (np.transpose(image_numpy, (1, 2, 0))) * 255.0
...
def save_image(image_numpy, image_path):
	...
	if image_numpy.shape[2] == 1:
		...
		# print(type(image_numpy),"sdffff")
		# print(image_numpy/255.0,"sdffff")
		# print(image_numpy.shape)
	else:
		image_pil = Image.fromarray(image_numpy)
		#image_numpy=image_numpy.astype(float)
		# print(image_numpy/255.0,"sdssssss")
		# print(image_numpy.shape)
