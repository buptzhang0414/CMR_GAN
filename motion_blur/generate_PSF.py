...
class PSF(object):
    def __init__(self, canvas=None, trajectory=None, fraction=None, path_to_save=None):
        ...
        if trajectory is None:
            self.trajectory = Trajectory(
                canvas=canvas, expl=0.005).fit(show=False, save=False)
        else:
    ...
    def fit(self, show=False, save=False):
        ...
        def triangle_fun(x): return np.maximum(0, (1 - np.abs(x)))

        def triangle_fun_prod(x, y): return np.multiply(
            triangle_fun(x), triangle_fun(y))
        for j in range(self.PSFnumber):
            ...
            for t in range(len(self.trajectory)):
                ...
                m2 = int(np.minimum(
                    self.canvas[1] - 1, np.maximum(1, np.math.floor(self.trajectory[t].real))))
                M2 = int(m2 + 1)
                m1 = int(np.minimum(
                    self.canvas[0] - 1, np.maximum(1, np.math.floor(self.trajectory[t].imag))))
                M1 = int(m1 + 1)
...
if __name__ == '__main__':
    psf = PSF(
        canvas=128, path_to_save='/Users/mykolam/PycharmProjects/University/RandomMotionBlur/psf.png')
