...
class ResnetGenerator(nn.Module):
    ...
    def forward(self, input):
        ...
        if self.learn_residual:
            ...
            output = input + output
            print(output,"uuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuu")
            #import torch.nn.functional as F
            #output = F.sigmoid(input + output)
            #output = torch.clamp(output, min=-1, max=1)
            # output = torch.clamp(output, min=-1, max=1)
            print(output, "eeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee")
        return output
...
class Edge(nn.Module):
    def __init__(self):
        ...
        self.grad_x = nn.Sequential(*layers)
        self.grad_x.training = False

        layers = []
        layers.append(nn.Conv2d(1, 1, 3, padding=1, bias=False))
        self.grad_y = nn.Sequential(*layers)
        self.grad_y.training = False

    def main(self,x):
        return self.grad_x(x),self.grad_y(x)
