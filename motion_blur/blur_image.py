    ...
    def blur_image(self, save=False, show=False):
        ...
        if len(psf) > 1:
            for p in psf:
                ...
                cv2.normalize(tmp, tmp, alpha=0, beta=1,
                ...
                blured = cv2.normalize(
                    blured, blured, alpha=0, beta=1, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_32F)
    ...
    def __plot_canvas(self, show, save):
        if len(self.result) == 0:
            ...
            if show and save:
                ...
                cv2.imwrite(os.path.join(self.path_to_save, self.image_path.split(
                    '/')[-1]), self.result[0] * 255)
                plt.show()
            elif save:
                if self.path_to_save is None:
...
if __name__ == '__main__':
    ...
    for path in os.listdir(folder):
        print(path)
        trajectory = Trajectory(canvas=64, max_len=60,
